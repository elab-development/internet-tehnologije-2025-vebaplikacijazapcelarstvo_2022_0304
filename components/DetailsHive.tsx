  ...
  async function handleDodajKomentar() {
    ...
    try {
      ...
      const response = await fetch("/api/comments", {
        ...
        body: JSON.stringify({
          sadrzaj: noviKomentar.trim(),
          kosnicaId: hive.id,
        }),
      });
      const data = await response.json();
      if (!response.ok)
        throw new Error(data.error || "Greška prilikom dodavanja komentara.");
      setNoviKomentar("");
  ...
  return (
    <>
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-4xl relative max-h-[90vh] overflow-y-auto">
          {/* Header */}
          <div className="flex items-center justify-between px-8 pt-7 pb-5 border-b border-gray-200 dark:border-gray-700">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <span
                  className={`inline-flex items-center px-3 py-0.5 rounded-full text-xs font-bold border ${getJacinaColor(hive.jacina)}`}
                >
                  {hive.jacina}
                </span>
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-gray-200 dark:divide-gray-700">
              ...
              {!hive.latitude && (
                <div className="px-8 py-4 border-t border-gray-200 dark:border-gray-700">
                  <p className="text-sm text-gray-400 dark:text-gray-500 text-center">
                    📍 Lokacija nije postavljena za ovu košnicu.
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="px-8 py-6 flex flex-col gap-4">
              ...
              <div>
                ...
                />
                {komentarGreska && (
                  <p className="text-red-500 text-xs mt-1">{komentarGreska}</p>
                )}
                <button
                      ...
                      <div className="flex items-center justify-between mb-1">
                        <div className="flex items-center gap-2">
                          <span className="font-semibold text-gray-800 dark:text-gray-100 text-xs">
                            {komentar.korisnik.ime}
                          </span>
                          <span
                            className={`text-xs px-2 py-0.5 rounded-full font-semibold border ${getJacinaColor(komentar.jacinaKosnice)}`}
                          >
                            {komentar.jacinaKosnice}
                          </span>
                        </div>
                        <span className="text-xs text-gray-400">
                          {new Date(komentar.createdAt).toLocaleDateString(
                            "sr-RS",
                            { day: "numeric", month: "short", year: "numeric" },
                          )}{" "}
                          {new Date(komentar.createdAt).toLocaleTimeString(
                            "sr-RS",
